...
CREATE PROCEDURE dbo.spr_dbt_reconq_discrepancy
...
DELETE ' + @Client_DB + '.dbo.dbt_reconq_discrepancy
WHERE plan_id = (SELECT TOP 1 PLAN_ID FROM ' + @DBNAME + '.dbo.tbl_plan_name)

INSERT INTO ' + @Client_DB + '.dbo.dbt_reconq_discrepancy
EXEC ' + @DBNAME + '.dbo.spr_dbt_reconq_discrepancy
...
SELECT * FROM dbo.dbt_reconq_discrepancy