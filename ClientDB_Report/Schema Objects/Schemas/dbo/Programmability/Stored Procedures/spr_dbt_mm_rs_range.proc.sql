...
CREATE PROCEDURE dbo.spr_dbt_mm_rs_range
...
DELETE ' + @Client_DB + '.dbo.dbt_mm_rs_range
WHERE plan_id = (SELECT TOP 1 PLAN_ID FROM ' + @DBNAME + '.dbo.tbl_plan_name)

INSERT INTO ' + @Client_DB + '.dbo.dbt_mm_rs_range
EXEC ' + @DBNAME + '.dbo.spr_dbt_mm_rs_range
...
SELECT * FROM dbo.dbt_mm_rs_range