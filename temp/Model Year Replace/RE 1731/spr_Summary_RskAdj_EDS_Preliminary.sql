...
from HRPReporting.dbo.lkRiskModelsMaster a
join [#Refresh_PY] b
...
SELECT DISTINCT
       ...
       [RAFactorType] = [icd].[RAFactorType],
	   [ModelVersion] = [icd].[ModelVersion],
       [ICDCode] = [icd].[ICD10CD],
       [HCCLabel] = [icd].[HCCLabel]
FROM HRPReporting.[dbo].lkRiskModelsDiagHCC [icd]
JOIN #lkRiskModelsMaster lkm
on icd.PaymentYear = lkm.PaymentYear
and icd.RAFactorType = lkm.RAFactorType
...
SELECT DISTINCT
       ...
       [ModelYear] = [split].[PaymentYear]
FROM [rev].[tbl_Summary_RskAdj_MMR] [mmr] WITH (NOLOCK)
    JOIN [#Refresh_PY] [py]
        ON [mmr].[PaymentYear] = [py].[Payment_Year]
    --JOIN [HRPReporting].[dbo].[lk_Risk_Score_Factors_PartC] [split] WITH (NOLOCK) -- manish -- Current Code
	JOIN #lkRiskModelsMaster [split] WITH (NOLOCK) -- new Code
...
BEGIN
    ...
    SELECT DISTINCT
           ...
           [Deleted] = CASE WHEN ISDelete = 1 THEN 'A'
							...
							ELSE NULL END , ---rri 1754
           [HCC_Label] = [hcc].[HCCLabel],
           [HCC_Number] = CAST(LTRIM(REVERSE(LEFT(REVERSE([hcc].[HCCLabel]), PATINDEX(
                                                                                          '%[A-Z]%',
                                                                                          REVERSE([hcc].[HCCLabel])
                                                                                      ) - 1)
                                            )
                                    ) AS INT),
...
*/
-- New Code
            ON [a].[ModelYear] = [hcc].[PaymentYear]
               AND [a].[PartCRAFTProjected] = [hcc].[RAFactorType]
               AND [a].[DiagnosisCode] = [hcc].[ICDCode]
...
EXEC	@return_value = [rev].[spr_Summary_RskAdj_EDS_Preliminary]
	@FullRefresh = 0,
    @YearRefresh = 2020,
    @LoadDateTime = NULL,
    @DeleteBatch = NULL,
