CREATE VIEW [edt].[EditConditionOverview] AS
SELECT 
	e.DVEEditID
	, e.EditEntity
	, e.EditQualifier
	...
	, isNULL(ec.ParentID,0) as ParentConditionID
	, ec.ConditionID as ConditionID
	, CASE WHEN isNULL(ec.isMet,0) = 1 THEN 'Positive' ELSE 'Negative' END as DesiredMatchState
	, CASE WHEN isNULL(ec.TrueOnANYChildCondition,0) = 1 THEN 'OR' ELSE 'AND' END as EditChildConditionGrouping
	, c.Name as ConditionName
	, c.Descr as ConditionDescr
	, vv.DataPath as ConditionEntity
	, tq1.Qualifier as ConditionQualifier
	, c.Value as ConditionValue
	, ra.Name as ConditionRangeName
	, ra.Descr as ConditionRangeDescr
	, ct.Name as ConditionCriteriaType
	, ct.Descr as ConditionCriteriaTypeDescr
	, fvm2.Message as EditConditionDefaultErrorMessage
	, c.FailedValidationMessage as EditConditionDefaultErrorMessageID
	, fvm.Message as EditConditionErrorMessage
	, ec.FailedValidationMessage as EditConditionErrorMessageID
...
FROM (
	...
	JOIN [edt].[EditCondition] ec ON e.DVEEditID = ec.EditID
	JOIN [edt].[Condition] c ON c.ID = ec.ConditionID
	LEFT JOIN [edt].[FailedValidationMessage] fvm ON fvm.ID = ec.FailedValidationMessage
...
WHERE e.NumberOfEditConditions > 0
	AND c.isActive = 1
